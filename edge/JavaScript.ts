...
const getPackageJsonList = memoize(async () => {
	...
	for (const packageJsonPath of packageJsonPathList) {
		...
		packageJsonList.push({
			packageJsonPath,
			nodeModulePathList: uniq(nodeModulePathList),
		})
	}

	return orderBy(packageJsonList, ({ packageJsonPath }) => fp.dirname(packageJsonPath).split(fp.sep).length, 'desc')
})
...
export default class JavaScript implements Language {
	...
	private async setFileCache(filePath: string, exportedIdentifierCache?: Map<FilePath, IdentifierMap>, fullPathCache?: { [fullPath: string]: boolean }) {
		...
		for (const [exportedName, identifier] of exportedIdentifiers) {
			...
			const list = union(this.transitExportTracer.get(item.traceID), [item])
...
function getInsertionPosition(existingImports: Array<ImportStatementForReadOnly>, path: string, document: vscode.TextDocument) {
	if (existingImports.length > 0) {
		if (path.startsWith('.')) {
			...
			const targetImport = findLast(fileImportList, stub => stub.path.localeCompare(path) === -1)
			if (targetImport) {
			...
			const targetImport = findLast(nodeImportList, stub => stub.path.localeCompare(path) === -1)
			if (targetImport) {
				return document.positionAt(targetImport.node.getEnd()).translate({ lineDelta: +1 }).with({ character: 0 })
...
async function getNodeJsAPIs(typeDefinitionPath: string): Promise<Array<string>> {
	...
	const fromOtherFiles = (await Promise.all(codeTree.referencedFiles.map(({ fileName }) => getNodeJsAPIs(fp.join(fp.dirname(typeDefinitionPath), fileName))))).flat()

	return difference(
		compact(union(fromCurrentFile, fromOtherFiles)),
