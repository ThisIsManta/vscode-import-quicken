...
const getPackageJsonList = memoize(async () => {
	...
	const pnpmWorkspaces = Object.fromEntries(compact(flatten(await Promise.all(
		pnpmLockPathList.map(async pnpmWorkspacePath => {
			const rootPath = fp.dirname(pnpmWorkspacePath)
			try {
				const { stdout } = await promisify(cp.exec)('pnpm list --recursive --depth -1 --parseable', { encoding: 'utf-8', cwd: rootPath })
	...
	// Represent a map of subdirectory to root directory in absolute paths
	const yarnWorkspaces = Object.fromEntries(compact(flatten(await Promise.all(
		yarnLockPathList.map(async yarnLockPath => {
			const rootPath = fp.dirname(yarnLockPath)
			try {
				const { stdout } = await promisify(cp.exec)('yarn --json workspaces info', { encoding: 'utf-8', cwd: rootPath })
	...
	const outputList = await Promise.all(
		packageJsonPathList.map(async packageJsonPath => {
			...
			const packageManager: 'npm' | 'pnpm' | 'yarn' | undefined = await (async () => {
		...
		if (!this.workingThread) {
			const λ = async () => {
				...
				for (const { packageManager, packageJsonPath, nodeModulePathList } of packageJsonList) {
					...
					const totalDependencyNameList = uniq(
				...
				for (const [packageJsonPath, identifiers] of this.nodeIdentifierCache) {
					this.nodeIdentifierCache.set(packageJsonPath, sortBy(identifiers,
...
async function getNodeJsTypeDefinitionFilePathFromRegistry(
	...
	const nodeVersion = (await (async () => {
		const run = (command: string) =>
			promisify(cp.exec)(command, { encoding: 'utf-8', cwd: workingDirectoryPath })
	...
	try {
		if (await fs.access(nodeTypeDirectoryPath).then(() => false, () => true)) {
			...
			const { versions } = await (await fetch('https://registry.npmjs.org/@types/node')).json() as { versions: Record<string, { dist: { tarball: string }, types: string }> }
			const { dist: { tarball } } = (
		...
		await fs.rm(nodeTypeDirectoryPath, { force: true, recursive: true })
	}
...
const getNodeJsAPIs = memoize(async (typeDefinitionFilePath: string): Promise<Array<string>> => {
	...
	return without(
		...
		'NodeJS', 'setTimeout', 'setImmediate'
	)
})
...
async function getDeclarationPath(moduleName: string, document: Pick<vscode.TextDocument, 'fileName'>, packageJsonList: Array<{ packageJsonPath: string, nodeModulePathList: Array<string> }>) {
	const packageJson = getClosestPackageJson(document.fileName, packageJsonList)
	if (!packageJson || !packageJson.nodeModulePathList) {
		return
	}
